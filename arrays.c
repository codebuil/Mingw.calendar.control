...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
       ...
       for(nn=0;nn<35;nn++){
        int nnn=40+nn;
        sprintf(startStr,"%d",nn);
        tthew[nn] = CreateWindow("BUTTON", startStr, WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
            (nn-((nn/7)*7))*50+5,(nn/7)*30+5, 50, 30,  hwnd, (HMENU)nnn, GetModuleHandle(NULL), NULL);
       }
